...
# Lucide 风格图标（24x24 viewBox，stroke）
ICONS = {
...
def main():
    ...
    frame_x, frame_y, frame_w, frame_h = 70, 292, 1460, 498
    ...
    gov_x, gov_w, gov_y, gov_h = 100, 1370, 662, 82
    ...
    S(f'<text x="{user_cx}" y="{user_cy + user_r + 16}" fill="{TEXT_DARK}" font-size="16" font-weight="bold" text-anchor="middle" dominant-baseline="central">用户</text>')

    # ---- 第一层：三交互模块 ----
    for i, (iname, title, sub) in enumerate(interactions):
        x = inter_xs[i]
        S(f'<rect x="{x - inter_w / 2}" y="{inter_cy - inter_h / 2}" width="{inter_w}" height="{inter_h}" rx="29" fill="{INTER_BG}" stroke="{INTER_STROKE}" stroke-width="1.5"/>')
        S(icon_title(x, inter_cy - 11, iname, title, 19, 19, 7, INTER_STROKE, TEXT_DARK))
        S(f'<text x="{x}" y="{inter_cy + 15}" fill="{TEXT_DIM}" font-size="14.5" text-anchor="middle" dominant-baseline="central">{sub}</text>')
    ...
    for i, (iname, title, items) in enumerate(blocks):
        ...
        S(icon_title(x + block_w / 2, block_y + 36, iname, title, 22, 21, 8, accent, accent))
    ...
    S(icon_title(gov_x + gov_w / 2, gov_y + 28, gov_icon, gov_title, 20, 19, 8, GOV_ACCENT, GOV_ACCENT))
    S(f'<text x="{gov_x + gov_w / 2}" y="{gov_y + 55}" fill="{TEXT_DIM}" font-size="16" text-anchor="middle" dominant-baseline="central">{gov_sub}</text>')
    ...
    for i, (iname, title, sub) in enumerate(bases):
        ...
        S(icon_title(x + base_w / 2, base_y + 26, iname, title, 20, 19, 8, BASE_STROKE, TEXT_DARK))
        S(f'<text x="{x + base_w / 2}" y="{base_y + 50}" fill="{TEXT_DIM}" font-size="15" text-anchor="middle" dominant-baseline="central">{sub}</text>')

    # ---- 底部说明 ----
    S(f'<text x="{W / 2}" y="946" fill="{TEXT_DIM}" font-size="16" text-anchor="middle">说明：系统以 AI 眼镜采集为入口，经素材库与 AI 生成沉淀为日记/游记，联动社区社交与内容治理，实现「采集→生成→发布→互动」完整闭环。</text>')
