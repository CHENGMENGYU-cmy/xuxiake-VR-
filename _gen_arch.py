...
ICONS = {
    ...
    "glasses": '<circle cx="6" cy="15" r="4"/><circle cx="18" cy="15" r="4"/><path d="M14 15a2 2 0 0 0-2-2 2 2 0 0 0-2 2"/><path d="M2 15h2M20 15h2M4 15l-2-6M20 15l2-6M6 9l3-4M18 9l-3-4"/>',
...
bases = [
    ("glasses", "智能眼镜", "摄像头/麦克风/扬声器/定位/通信"),
    ("layers", "应用层", "Next.js + React · NestJS"),
    ("zap", "实时通信", "WebSocket · SSE"),
    ("database", "数据与AI", "MySQL 8 · DeepSeek（暂时个人API）"),
...
def main():
    ...
    user_cx, user_cy, user_r = 800, 112, 36
    beam1_y = 168
    inter_cy = 232
    inter_h = 60
    inter_w = 224
    ...
    for i, (iname, title, sub) in enumerate(bases):
        ...
        S(icon_title(x + base_w / 2, base_y + 27, iname, title, 21, 20, 7, BASE_STROKE, TEXT_DARK))
        S(f'<text x="{x + base_w / 2}" y="{base_y + 52}" fill="{TEXT_DIM}" font-size="14" text-anchor="middle" dominant-baseline="central">{sub}</text>')

    # ---- 底部说明 ----
    S(f'<text x="{W / 2}" y="936" fill="{TEXT_DIM}" font-size="17" text-anchor="middle">说明：系统以智能眼镜采集为入口，经素材库与 AI 生成沉淀为日记/游记，联动社区社交与内容治理，实现「采集→生成→发布→互动」完整闭环。</text>')
