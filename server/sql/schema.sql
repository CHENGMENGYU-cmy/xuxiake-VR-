...
CREATE TABLE IF NOT EXISTS users (
  ...
  status            ENUM('ACTIVE','BANNED','DEACTIVATED') NOT NULL DEFAULT 'ACTIVE' COMMENT '状态',
  deactivated_at    TIMESTAMP NULL DEFAULT NULL COMMENT '注销时间',
  deactivation_reason TEXT NULL COMMENT '注销原因',
...
CREATE TABLE IF NOT EXISTS posts (
  ...
  post_type      ENUM('NOTE','VR_MEDIA','JOURNEY','MOMENT') NOT NULL DEFAULT 'NOTE' COMMENT '帖子类型',
  community_id   VARCHAR(36) NULL COMMENT '关联社群ID',
  content_level  ENUM('SNAPSHOT','POST','CLASSIFIED','DIARY','TRAVELOGUE','ESSAY') NOT NULL DEFAULT 'POST' COMMENT '内容层级：SNAPSHOT闪拍|POST随笔|CLASSIFIED旧分类帖|DIARY日记|TRAVELOGUE游记|ESSAY旧游记',
