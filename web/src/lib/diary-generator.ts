import type { DiaryStyle, GeneratedDiary, SnapRecord } from '@/types/snap';

/** 根据闪拍内容推荐日记风格（前端备用） */
export function recommendStyle(record: SnapRecord): DiaryStyle {
  const text = (record.content || '') + (record.keywords || []).join('');
...
/** 前端模拟生成日记（离线模式备用） */
export function generateDiaryLocal(
  record: SnapRecord,
  style: DiaryStyle,
  includeMemory = false,
): GeneratedDiary {
  ...
  const snapText = record.content || '';
  const location = record.location || '某个地方';
  const time = record.createdAt || '某个时刻';
  const keywords = (record.keywords || []).join('、') || '日常';
