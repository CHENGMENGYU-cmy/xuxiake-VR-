...
第1条
  ...
  修改时间：2026-07-16 22:00
  修改位置：web/src/app/(landing)/page.tsx
  修改原因：参照 DeepSeek 风格重新设计落地页，实现完整的访问流程：介绍页→登录→社区
  修改内容：
    - 落地页改为 'use client' 组件，接入 useAuthStore 检测登录状态
    - 已登录用户访问 / 自动跳转到 /feed，不显示介绍内容
    - 重新设计 Hero 区域：全屏高度、多层渐变背景、动态光晕、网格点阵
    - 核心理念板块：三列卡片，hover 动效增强
    - 功能展示：四大功能左右交替布局，每个功能配可视化演示区域
    - 数据佐证：卡片式统计展示，hover 动效
    - 平台愿景：独立板块，强调理念
    - 底部 CTA：渐变背景，双按钮引导注册/登录
    - 顶部导航增加锚点链接（功能/理念/社区）
    - 页脚增加导航链接
  修改效果：完整的用户访问流程——访问 / 看到丰富的介绍页，点击"开始探索"进入登录，登录后进入社区不再看到介绍
