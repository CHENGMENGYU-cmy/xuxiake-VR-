...
第1条
  ...
  修改时间：2026-07-16 21:20
  修改位置：web/src/app/(landing)/page.tsx, web/src/app/page.tsx
  修改原因：根路由直接重定向到 /feed，缺少独立的落地介绍页，需要一个类似 DeepSeek 风格的首页
  修改内容：
    - 创建 (landing) route group，新增落地页作为根路由 /
    - 删除旧的 page.tsx（redirect 到 /feed）
    - 落地页包含：顶部导航栏、Hero 区域、三大核心理念、四个功能展示（左右交替布局）、数据佐证、底部 CTA、页脚
    - 所有 CTA 按钮指向 /login，引导用户开始使用
    - 使用品牌渐变、lucide 图标、shadcn Badge 组件
  修改效果：用户访问 / 首先看到完整的平台介绍页，点击按钮跳转登录后进入社区
