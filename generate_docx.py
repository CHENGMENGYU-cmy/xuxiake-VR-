...
doc.add_heading('5.0.2 核心判断：分两条轨道推进', level=3)
add_body('轨道A：可上线版本（没有 AR 硬件也能跑）')
add_bullet('目标：8月31日前推出徐霞客社区 Beta 版')
add_bullet('依赖：不需要 AR 眼镜，不需要新硬件')
add_bullet('可能性：★★★★★（非常高）')

add_body('轨道B：AR 增强版本（需要硬件 + 深度开发）')
add_bullet('目标：10月后陆续接入')
add_bullet('依赖：AR 眼镜设备、WebXR 浏览器支持')
add_bullet('可能性：★☆☆☆☆（开学前不可能完成）')

add_body('关键结论：不要在开学前死磕 AR。先把已有的能力打磨好推出去。')

doc.add_heading('5.0.3 开学前6周冲刺计划', level=3)
...
doc.add_heading('5.0.4 开学前能达到的状态', level=3)
add_styled_table(
    ['维度', '预期状态'],
    [
        ['平台可用性', '完全可用——注册、发帖、互动、聊天、社群全部正常'],
        ['内容质量', '有种子数据 + 早期用户内容，不显得空'],
        ['性能', '首屏 < 3s，API < 200ms，可承载 200+ 并发'],
        ['AR 能力', 'VR 内容上传/播放已支持（模拟数据），真机 AR 接入未完成'],
...
doc.add_heading('5.0.5 诚实结论', level=3)
add_body('8月31日前推出徐霞客社区 Beta 版的可能性：90%以上。前提是：聚焦打磨现有功能+部署上线，不在此阶段引入 AR 真机对接。')
add_body('做不到的事情：真正的 AR 眼镜实时数据采集、WebXR 沉浸式浏览、高精度个性化推荐——这些需要硬件支持、浏览器生态成熟、以及大量用户行为数据，不是6周能解决的问题。')
add_body('但好消息是：即使没有 AR 硬件，徐霞客社区作为一个以 VR 内容为核心的旅行社交平台，核心体验已经完整。VR360/VR180 视频上传和播放、攻略创建、路线记录、社群活动这些特色功能，已经足以和现有旅行平台形成差异化。')

doc.add_heading('5.0.6 如果做不到——风险降级与兜底方案', level=3)
