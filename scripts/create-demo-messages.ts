...
async function main() {
  ...
  try {
    // 获取两个用户
    const [users] = await connection.query('SELECT id, username, displayName FROM users LIMIT 2') as any;
    ...
    // 检查是否互相关注
    const [followAB] = await connection.query(
      'SELECT * FROM user_follows WHERE follower_id = ? AND following_id = ?',
      [userA.id, userB.id]
    );
    const [followBA] = await connection.query(
      'SELECT * FROM user_follows WHERE follower_id = ? AND following_id = ?',
      [userB.id, userA.id]
    );

    const isMutual = (followAB as any).length > 0 && (followBA as any).length > 0;
