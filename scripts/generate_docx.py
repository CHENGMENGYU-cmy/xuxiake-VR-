...
# ── 5.0 开学前可行性评估 ──
doc.add_heading('5.0 开学前可行性评估（2026.7.20 — 8.31，约6周）', level=2)

doc.add_heading('5.0.1 冷静评估：我们手里有什么', level=3)
add_body('在制定宏大计划之前，先诚实地盘点当前状态。好消息是——徐霞客社区已经是一个可以运行的旅行社交平台了。')
...
doc.add_heading('5.0.2 重大更新：AR 眼镜数据已就绪', level=3)
add_body('7月20日更新：AR 眼镜真机数据已到位。此前评估中"AR 眼镜真机数据接入"是最大未知数（0%，需要硬件），现在这个瓶颈已消除。AR 数据接入可以从研究阶段直接进入开发阶段。')

doc.add_heading('5.0.3 核心判断：三条工作线并行', level=3)
add_body('工作线A：平台上线（打磨+测试+部署）')
add_bullet('目标：8月31日前推出徐霞客社区 Beta 版')
add_bullet('依赖：无外部依赖，纯软件工程')
add_bullet('可能性：★★★★★（非常高）')

add_body('工作线B：AR 数据接入（导入+存储+展示）')
add_bullet('目标：8月31日前完成 AR 数据的基础接入管线')
add_bullet('依赖：AR 眼镜数据已就绪')
add_bullet('可能性：★★★★☆（高，6周足够搭好管线）')

add_body('工作线C：AR 沉浸式渲染（WebXR + 3D场景）')
add_bullet('目标：10月后陆续接入')
add_bullet('依赖：需要先积累 AR 内容，依赖浏览器生态成熟')
add_bullet('可能性：★☆☆☆☆（开学前来不及，也没必要赶）')

add_body('关键结论：AR 数据来了，应该立刻启动接入开发。但不要贪多——"接入数据"和"沉浸式渲染"是两回事，先把数据跑通。')

doc.add_heading('5.0.4 开学前6周冲刺计划', level=3)

add_body('【平台上线线——工作线A】')

add_body('第1-2周（7.20-8.2）：打磨收尾')
add_styled_table(
    ['事项', '具体内容', '工作量'],
    [
...
add_body('【AR 数据接入线——工作线B，与上线线并行推进】')
add_styled_table(
    ['阶段', '时间', '事项', '具体内容', '工作量'],
    [
        ['数据定义', 'W1', 'AR 数据格式梳理', '根据已有真实数据，定义标准化导入格式（轨迹JSON、媒体文件、传感器日志）', '2天'],
        ...
        ['集成联调', 'W5-W6', '前后端联调+测试', 'AR 数据导入→自动生成帖子→信息流展示→点击查看完整链路跑通', '2天'],
    ]
)
add_body('注意：工作线B的目标是"数据能导入、能存储、能展示"，不是"完美沉浸式体验"。WebXR 渲染等高级功能放在开学后。')

doc.add_heading('5.0.5 开学前能达到的状态', level=3)
add_styled_table(
    ['维度', '预期状态'],
    [
...
doc.add_heading('5.0.6 诚实结论', level=3)
add_body('8月31日前推出徐霞客社区 Beta 版的可能性：90%以上。')
add_body('AR 数据接入基础管线的可能性：85%以上。（数据已就绪，纯工程实现，6周足够搭好导入→存储→展示的完整链路）')
add_body('前提是：三线并行——平台打磨+AR数据接入+部署上线同时推进，各司其职。')
add_body('做不到的事情：WebXR 沉浸式渲染、高精度个性化推荐——前者需要积累足够的 AR 内容才有渲染的意义，后者需要大量用户行为数据。')
add_body('最大的变化：之前因为没硬件，AR 接入的可能性是 0%。现在数据已到位，AR 不再是"未来规划"而是"正在做的事"。Beta 版上线时就能展示真实 AR 眼镜数据——这比纯模拟数据的平台多了一个实实在在的差异化亮点。')

doc.add_heading('5.0.7 如果做不到——风险降级与兜底方案', level=3)
add_body('90% 的可能性意味着还有 10% 的失败空间。以下是可能导致开学前无法推出的真实场景，以及对应的降级策略。')

doc.add_heading('场景一：部署遇到不可逾越的障碍', level=3)
add_styled_table(
    ['项目', '说明'],
    [
...
doc.add_heading('场景二：测试阶段发现严重 Bug', level=3)
add_styled_table(
    ['项目', '说明'],
    [
...
doc.add_heading('场景三：人手不足或时间被挤压', level=3)
add_styled_table(
    ['项目', '说明'],
    [
...
doc.add_heading('场景四：开学前完全无法交付公网版本', level=3)
add_styled_table(
    ['项目', '说明'],
    [
...
doc.add_heading('场景五：AR 数据接入进度滞后', level=3)
add_styled_table(
    ['项目', '说明'],
    [
        ['具体表现', 'AR 数据的格式比预期复杂、字段缺失或格式不统一导致解析器开发卡住'],
        ['降级方案', '先支持最核心的轨迹数据（GPS坐标+时间戳），媒体和传感器数据逐步接入；手工清洗一份标准样本数据作为第一批导入内容'],
        ['底线交付', '至少能在平台上展示一条完整的 AR 眼镜轨迹——哪怕只有一条，证明管线是通的'],
        ['影响评估', 'AR 功能范围缩小，但不影响平台主体上线。管线通了之后，剩下的就是数据量的积累'],
    ]
)

doc.add_heading('降级决策树——什么时候该放弃原计划？', level=3)
add_body('8月15日检查点（AR）：AR 数据基础管线跑通了吗？')
add_bullet('能 → 继续推进可视化和集成 → Beta 版含 AR 数据展示')
add_bullet('不能 → 缩小范围：只做轨迹导入 → 其余数据开学后补充')
add_body('8月25日检查点（部署）：后端能在公网访问吗？')
add_bullet('能 → 继续推进前端部署 + 公测 → 有可能 8.31 上线')
add_bullet('不能 → 进入 8月28日最终检查点')
add_body('8月28日最终检查点：')
add_bullet('本地环境完整可用 → 录屏演示 + 文档 → 开学后第一周上线')
add_bullet('本地环境也有问题 → 回退到最稳定版本 → 逐一修复 → 9月中旬上线')

add_body('一句话总结：最差情况下，至少保证一套可在本地完整演示的系统（含至少一条 AR 轨迹数据）+ 一份详尽的项目报告。不存在"什么都没做出来"的情况——平台代码已写好，AR 数据已到手，只是做多做少的问题。')

doc.add_heading('5.1 总体时间线——两阶段战略', level=2)
add_body('核心原则：暑假结束前完成基本功能开发并上线 Beta 版；开学后利用课余时间持续优化，不设硬性 deadline。')

add_styled_table(
    ['', '阶段一：暑假冲刺', '阶段二：开学后持续优化'],
    [
        ['时间', '2026.7.20 — 8.31（6周）', '2026.9 — 2027.5（8个月）'],
        ['目标', '基本开发完成 + Beta 上线', '持续迭代优化'],
        ['AR 数据', '导入→存储→展示基础管线', '沉浸式渲染（WebXR + 3D场景）'],
        ['社区', '挑战系统补全（排行榜+计分）', '徽章成就 + 社群地图 + 内容策展'],
        ['推荐', '基础多信号加权（已有）', '协同过滤 + A/B测试 + 实时更新'],
        ['平台', '打磨 + 测试 + 部署上线', 'Redis缓存 + PWA + 移动端深度适配'],
        ['运营', 'Beta 版限量公测', '小范围推广 → 收集反馈 → 扩大推广'],
        ['人力', '现有 1-2 人全职', '课余时间开发'],
    ]
)

doc.add_heading('5.2 阶段一：暑假冲刺（2026.7.20 — 8.31）', level=2)
add_body('目标：完成基本开发 + 部署上线 Beta 版，核心功能可用。详细周计划见 5.0.4 节。')

add_styled_table(
    ['时间节点', '里程碑', '检查标准'],
    [
        ['7.26（第1周末）', 'AR 数据格式定义完成', '标准化导入格式文档 + 数据样本通过校验'],
        ['8.2（第2周末）', '平台打磨收尾', '社群挑战补全、推荐调优、移动适配基本完成'],
        ['8.9（第3周末）', 'AR 数据存储管线跑通', '真实 AR 数据可导入数据库，API 可查询'],
        ['8.16（第4周末）', '测试加固完成', '核心链路测试通过、性能达标、安全无高危漏洞'],
        ['8.23（第5周末）', 'AR 可视化初步可用', '轨迹在地图上展示、帖子详情页含 AR 数据摘要'],
        ['8.31（第6周末）', 'Beta 版上线', '公网可访问、AR 数据可导入可展示、核心功能闭环'],
    ]
)

add_body('暑假结束时的交付物：一个公网可访问的徐霞客社区 Beta 版、AR 眼镜数据导入→存储→展示的基础管线、社群挑战系统（含排行榜和自动计分）、测试报告 + 性能基准。')

doc.add_heading('5.3 阶段二：开学后持续优化（2026.9 — 2027.5）', level=2)
add_body('开学后无全职开发时间，利用课余和周末推进。每个模块独立迭代，不设严格顺序。')

doc.add_heading('模块A：AR 沉浸式渲染（优先级：中）', level=3)
add_styled_table(
    ['时间', '工作内容', '交付物'],
    [
        ['9月-10月', 'WebXR 集成调研与原型', 'WebXR Demo'],
        ['11月-12月', 'Three.js/CesiumJS 3D 场景渲染', 'AR 3D 浏览页'],
        ['2027.1-3月', 'AR 标记点系统 + 多平台适配', '完整 AR 渲染能力'],
    ]
)

doc.add_heading('模块B：推荐算法优化（优先级：中）', level=3)
add_styled_table(
    ['时间', '工作内容', '交付物'],
    [
        ['9月-10月', '行为信号采集系统上线', '用户行为数据开始积累'],
        ['11月-12月', '混合推荐算法实现 + A/B 测试', '推荐效果提升'],
        ['2027.1-2月', '实时推荐更新 + 负反馈学习', '个性化推荐系统'],
    ]
)

doc.add_heading('模块C：社区功能深化（优先级：中高）', level=3)
add_styled_table(
    ['时间', '工作内容', '交付物'],
    [
        ['9月-10月', '成就徽章系统 + 社群地图', '徽章系统 + 地图可视化'],
        ['11月-12月', '内容策展 + 攻略路线共享', '策展平台'],
        ['2027.1月起', '根据数据反馈持续迭代', '—'],
    ]
)

doc.add_heading('模块D：运营推广（优先级：开学后逐步启动）', level=3)
add_styled_table(
    ['时间', '工作内容'],
    [
        ['9月', '小范围邀请注册（同学/朋友），收集反馈'],
        ['10月-11月', '根据反馈快速迭代，修复高频问题'],
        ['12月起', '逐步扩大推广，沉淀种子用户'],
    ]
)

doc.add_heading('模块E：性能与质量（优先级：贯穿始终）', level=3)
add_styled_table(
    ['时间', '工作内容'],
    [
        ['9月', 'Redis 缓存层接入'],
        ['10月-11月', 'PWA 支持、移动端深度适配'],
        ['持续', '性能监控、安全审计、Bug 修复'],
    ]
)

doc.add_heading('5.4 关键决策点', level=2)
add_styled_table(
    ['时间', '决策', '判断依据'],
    [
        ['8.15', 'AR 数据管线是否达到可用标准？', '决定 Beta 版包含多少 AR 能力'],
        ['8.28', '是否具备公网上线条件？', '决定 8.31 上线 or 延期到 9 月中旬'],
        ['9.30', '开学后开发节奏评估', '根据课程安排调整后续计划'],
        ['12.31', '是否进入公开推广？', '根据用户反馈和数据决定推广力度'],
    ]
)

doc.add_heading('5.5 资源需求', level=2)
add_body('暑假冲刺期（阶段一）仅需现有人员——1-2 名开发者，已有代码基础和 AR 数据。无需额外硬件。')
add_body('开学后（阶段二）以课余开发为主，不设专职人力要求。深度功能（WebXR、高级算法）可在时间充裕时逐步推进。')
add_body('注意：资源需求较此前大幅简化——因为暑假已完成基本开发，开学后主要是迭代优化而非从零建设。')
...
# ── 保存 ──
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'docs', '20-汇报材料', '研究计划与研究方案.docx')
